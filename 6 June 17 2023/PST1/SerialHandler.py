...
def connect_serial(comport,baudrate,beambreakevents, beambreak_dic):
    global ser
    ...
    while reading_serial:
        if ser.in_waiting >= 3:
            ...
            if bytes_read == b"@es":
                ...
                dispense_event.set()
                beambreak_dic.update({"BBtime":dispense_time})
                beambreak_dic.update({"candy_disp":1})
                beambreak_dic.update({"candy_disp":0})
                #return('candy_disp')
                ...
                beambreakevents_var.write('%0.3f,%i,%i\n' %(taken_time, 0, 1))
                beambreak_dic.update({"BBtime":taken_time})
                beambreak_dic.update({"candy_disp":0})
                beambreak_dic.update({"candy_disp":1})
                #return('candy_taken')
                #return taken_time
