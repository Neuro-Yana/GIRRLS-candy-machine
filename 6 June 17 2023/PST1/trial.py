...
#pk['beambreakevents'].update({[None]})
...
if info['Bluetooth'] == False:    
    SerialHandler.connect_serial(info['Com Port'],baud,beambreakevents_file, pk['beambreakevents'])
...
if ready == True:
    ...
    clean_quit(datapath, pk, info['participant'], block_clock,beambreakevents_file)
...
#why # of trials is trials*num_stims
...
# create a experiments parameters dataframe and save   
experiments_parameters_df = make_df_experiments_parameters(pk)
savepath_experiments_parameters = os.path.join(datapath, '%s'%info['participant']+"_experiments_parameters",'%s.csv'%info['participant'])
experiments_parameters_df.to_csv(savepath_experiments_parameters, index=False)

#save data
info_df = make_df_info(pk, info['participant'])
savepath_info = os.path.join(datapath, '%s'%name+"_info",'%s.csv'%name)
