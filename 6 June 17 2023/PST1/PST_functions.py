...
def clean_quit(basepath, dic, name, block_clock, beambreakevents_file):
    ...
    savepath_experiment_parameters = os.path.join(basepath, '%s'%name+"_experiments_parameters",'%s.csv'%name)
    experiment_parameters_df.to_csv(savepath_experiment_parameters, index=False)
    #saving info
    info_df = make_df_info(dic, name)
    savepath_info = os.path.join(basepath, '%s'%name+"_info",'%s.csv'%name)
    info_df.to_csv(savepath_info, index=False)
