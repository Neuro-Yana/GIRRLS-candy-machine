...
example_data = pk[participant_ID]['data'][block]
...
def make_df_experiment_parameters(dic):
    expparams_list = []
    expparams_df = pd.DataFrame.from_dict(dic['experiment_parameters']) 
    expparams_list.append(expparams_df)
    expparams_df_output = pd.concat(expparams_list, axis=0)
    return(expparams_df_output)

def make_df_info(dic, name):
    info_list = []
    info_dic = dic[str(name)] 
    info_df = pd.DataFrame.from_dict(info_dic, orient = "index")
    info_list.append(info_df)
    info_df_output = pd.concat(info_list, axis=0)
    return(info_df_output)

      
def present_stims(fix,left_stim, right_stim, win, left_key,right_key,quit_key, RT, block_clock, scheduled_outcome):
...
def response_update(key_pressed, win, left_stim, right_stim, left_choice, right_choice, block_clock):  
    resp_onset = block_clock.getTime()
    if key_pressed == '1':
        left_choice.draw()
    elif key_pressed == '4':
        right_choice.draw()
...
def show_resp(key_pressed, left_choice, right_choice, block_clock):
    resp_onset = block_clock.getTime()
    if key_pressed == '1':
        left_choice.draw()
        # win.flip()
...
def show_fdbk(accuracy,sched_out,block_clock, zero, win, reward, X):

    fdbk_onset = block_clock.getTime()
...
def clean_quit(basepath, dic, name, block_clock, beambreakevents_file, beambreakevents_dict):
    dic.update({'quit':block_clock.getTime()})
    beambreakevents_file.close()
    beambreakevents_dic = SerialHandler.close_serial()
    dic['beambreakevents'] = {}
    dic.update({'beambreakevents':beambreakevents_dic})
    adillyofapickle(basepath, dic, name)
    print(dic)
    #df = make_df(dic)
    #savepath = os.path.join(basepath,'%s'%name,'%s.csv'%name)
    #df.to_csv(savepath, index=False)
    # create a dataframe and save   
    data_df = make_df_data(dic)
    savepath_data = os.path.join(basepath,'%s'%name,'%s.csv'%name)
    data_df.to_csv(savepath_data, index=False)
    # create an experiment parameters dataframe and save   
    experiment_parameters_df = make_df_experiment_parameters(dic)
    experiment_parameters_output_file = str('%s' %name + '_experiment_parameters' + '.csv')
    experiment_parameters_savepath = Path(os.path.join(basepath, '%s'%name))
    experiment_parameters_df.to_csv(experiment_parameters_savepath/experiment_parameters_output_file, index=False)
    #saving info
    info_df = make_df_info(dic, name)
    info_output_file = str('%s' %name + '_info' + '.csv')
    info_savepath = Path(os.path.join(basepath, '%s'%name))
